...
func newRes(code int, msg string, data ...interface{}) Result {
	if len(data) > 0 {
		return Result{
			Code: code,
			Msg:  msg,
...
func NewPage(msg string, items interface{}, count int) Result {
	return Result{
		Code: stSucc,
		Msg:  msg,
		Data: page{
...
func NewErrIpt(msg string, data ...interface{}) Result {
	return newRes(stErrIpt, msg, data...)
}

//NewErrOpt 返回一个输出错误的结果格式
func NewErrOpt(msg string, data ...interface{}) Result {
	return newRes(stErrOpt, msg, data...)
}

//NewErrDeny 返回一个没有权限的结果格式
func NewErrDeny(msg string, data ...interface{}) Result {
	return newRes(stErrDeny, msg, data...)
}

//NewErrJwt 返回一个通过验证的结果格式
func NewErrJwt(msg string, data ...interface{}) Result {
	return newRes(stErrJwt, msg, data...)
}

//NewErrSvr 返回一个服务端错误的结果格式
func NewErrSvr(msg string, data ...interface{}) Result {
