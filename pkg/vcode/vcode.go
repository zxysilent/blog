...
func New(ln int, secret string) *Field {
	...
	fmt.Println(str)
	v := &Field{}
	v.Image = newImage(str).Base64()
	hm := hmac.New(md5.New, []byte(secret))
	hm.Write([]byte(str))
	// enc.ms.sign
	enc := base64.RawURLEncoding.EncodeToString(hm.Sum(nil))
	hm.Reset()
	now := time.Now().Add(60 * time.Second).Unix()
	fmt.Fprintf(hm, "%s.%d", enc, now)
	sign := base64.RawURLEncoding.EncodeToString(hm.Sum(nil))
	v.Real = fmt.Sprintf("%s.%d.%s", enc, now, sign)
	return v
}
func Check(secret string, real string, code string) error {
	...
	fmt.Fprintf(hm, "%s.%s", parts[0], parts[1])
	if base64.RawURLEncoding.EncodeToString(hm.Sum(nil)) != parts[2] {
		return errors.New("非法输入")
	}
	...
	hm.Reset()
	hm.Write([]byte(code))
	if base64.RawURLEncoding.EncodeToString(hm.Sum(nil)) != parts[0] {
