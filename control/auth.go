...
func UserLogin(ctx echo.Context) error {
	...
	err := ctx.Bind(&ipt)
	if err != nil {
		return ctx.JSON(xutil.NewErrIpt(`请输入用户名和密码`, err.Error()))
	}
	if ipt.Num == "" && len(ipt.Num) > 18 {
		return ctx.JSON(xutil.NewErrIpt(`请输入正确的用户名`))
	}
	mod, has := model.UserByNum(ipt.Num)
	if !has {
		return ctx.JSON(xutil.NewErrOpt(`用户名输入错误`))
	}
	now := time.Now()
	// 禁止登陆证 5 分钟
	if mod.Ecount == -1 {
		...
		if span >= 1 { //「」
			return ctx.JSON(xutil.NewFail(`请「` + strconv.Itoa(span) + `」分钟后登录`))
		}
		mod.Ecount = 0
	}
	if mod.Pass != ipt.Pass {
		...
		if mod.Ecount >= 3 {
			...
			return ctx.JSON(xutil.NewFail(`登录锁定请「5」分钟后登录`))
		}
		// 小于3 提示剩余次数
		model.UserEditLogin(mod, "Ltime", "Ecount")
		return ctx.JSON(xutil.NewFail(`密码错误,剩于登录次数：` + strconv.Itoa(int(3-mod.Ecount))))
	}
	if !mod.Role.IsAtv() {
		return ctx.JSON(xutil.NewFail(`当前账号已被禁用`))
	}
	...
	if err != nil {
		return ctx.JSON(xutil.NewFail(`凭证生成失败,请重试`, err.Error()))
	}
	mod.Ltime = now
	mod.Ip = ctx.RealIP()
	model.UserEditLogin(mod, "Ltime", "Ip", "Ecount")
	return ctx.JSON(xutil.NewSucc(`登陆成功`, jwtStr))
}
