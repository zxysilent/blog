...
func PostPath(path string) (*Post, *Naver, bool) {
	...
	if has {
		...
		naver := &Naver{}
		p := Post{}
		b, _ := Db.Where("Type = 0 and Is_Public = 1 and Status = 3 and Create_Time <?", mod.CreateTime.Format(utils.StdDateTime)).Desc("Create_Time").Get(&p)
		if b {
			// <a href="{{.Naver.Prev}}" class="prev">&laquo; 上一页</a>
			naver.Prev = `<a href="/post/` + p.Path + `.html" class="prev">&laquo; ` + p.Title + `</a>`
		}
		n := Post{}
		b1, _ := Db.Where("Type = 0 and Is_Public = 1 and Status = 3 and Create_Time >?", mod.CreateTime.Format(utils.StdDateTime)).Asc("Create_Time").Get(&n)
		if b1 {
