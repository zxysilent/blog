            ...
            <Row :gutter="2">
                <Col span="20">
                    <Form ref="dataForm" :rules="dataRules" :model="dataForm">
                        ...
                        <FormItem style="margin-bottom:15px" prop="path">
                            <Row>
                                ...
                                <Col span="8"><Input type="text" v-bind:disabled="isEdit" v-model="dataForm.path"
                                                     placeholder="请输入访问路径"></Input>
                                </Col>
                                <Col span="10" style="min-width:250px"> .html&nbsp;
                                    <a title="预览" @click="clkPreview">
                    ...
                    <div style="min-height: 600px;height: calc(100vh - 145px);">
                        <button ref="diy" type="button" @click="diyMore" class="op-icon ivu-icon ivu-icon-md-code"
                                aria-hidden="true" title="更多"></button>
                        <mavon-editor ref="md" @imgAdd="imgAdd" :boxShadow="false" @change="change" :toolbars="toolbars"
                                      v-model="dataForm.markdown_content" style="height:100%">
                        </mavon-editor>
                    </div>
                </Col>
                <Col span="4">
                        ...
                        <Form ref="optsForm" label-position="top">
                            <FormItem v-if="isPost" label="分类">
                                <RadioGroup v-model="dataForm.cate_id">
                                    <Radio label="cate" v-for="item in cateAll" :label="item.id" :key="item.id">
                            ...
                            <FormItem label="排序ID">
                                <Input v-model.number="dataForm.sort" placeholder="请输入排序ID"/>
                            </FormItem>
                            <FormItem label="发布日期">
                                <DatePicker v-model="dataForm.create_time" type="datetime" placeholder="选择发布日期和时间"
                                            :clearable="false" :editable="false"></DatePicker>
                            </FormItem>
                            <FormItem v-if="isPost" label="标签">
                                <Select v-model="tags" multiple placeholder="请选择文章标签">
                                    <Option v-for="item in tagAll" :value="item.id" :key="item.id">{{ item.name }}
                                    </Option>
                                </Select>
                            </FormItem>
                            <br><br>
...
<script>
    ...
    import {admPostOpts, apiPostGet, apiPostTagGet} from "@/api/post";
    // 通用 文章/页面 + 添加/修改
    // 减少js体积
    export default {
        components: {
